...
# 2) Labels
label_names = [
    "rule_of_thirds","leading_lines","framing","symmetry",
    "patterns","golden_ratio","layers","negative_space",
    "fill_the_frame"
]
...
model.load_state_dict(torch.load("composition/composition_model.pth", map_location=device))
model.eval()
...
def predict_composition(image_path: str,
                  threshold: float = 0.5
                  ):
    # a) Network inference
...
if __name__ == "__main__":
    test_path = "test.jpg"
    preds     = predict_composition(test_path, threshold=0.3)
    print("\nPredicted composition labels:", preds)