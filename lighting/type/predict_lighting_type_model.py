...
# Label names
label_names = ["natural_light","artificial_light"]

# ──────────────────────────────────────────────────────────────────────────────
# 3) Load trained model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model  = MultiLabelResNet(num_classes=len(label_names)).to(device)
model.load_state_dict(torch.load("lighting/type/lighting_type_model.pth", map_location=device))
model.eval()
...
def predict_light_type(image_path: str,
                  threshold: float = 0.5
                  ):
    # a) Network inference
