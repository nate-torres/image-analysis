...
# Label names
label_names = ["hard_light","soft_light"]

# ──────────────────────────────────────────────────────────────────────────────
# 3) Load trained model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model  = MultiLabelResNet(num_classes=len(label_names)).to(device)
model.load_state_dict(torch.load("lighting/quality/lighting_quality_model.pth", map_location=device))
model.eval()
...
def predict_light_quality(image_path: str,
                  threshold: float = 0.5
                  ):
    # a) Network inference
