...
# Label names
label_names = ["split_lighting","loop_lighting","rembrandt_lighting","butterfly_lighting","flat_lighting","silhouette"]

# ──────────────────────────────────────────────────────────────────────────────
# 3) Load trained model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model  = MultiLabelResNet(num_classes=len(label_names)).to(device)
model.load_state_dict(torch.load("lighting/pattern/lighting_pattern_model.pth", map_location=device))
model.eval()
...
def predict_light_pattern(image_path: str,
                  threshold: float = 0.5
                  ):
    # a) Network inference
